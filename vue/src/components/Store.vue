<template>
  <section class="container mx-auto pt-14">
    <nav id="store" class="w-full z-30 top-0 px-6 py-1">
      <div
        class="w-full container mx-auto flex flex-col lg:flex-row items-center justify-between mt-0 px-2 py-3 space-y-6 lg:space-y-0"
      >
        <a
        ...
        <div v-if="loadingCategories">
          <p>makayna 7ta categories</p>
        </div>
        <div
          v-else
          class="flex divide-x divide-black divide-solid text-xs xl:text-md"
        >
          <div v-for="(category, index) in categories" :key="index">
            ...
            >
              <p
                class="px-2 lg:px-4 hover:underline"
                :class="activeCategory == category ? 'font-bold' : ''"
              >
                {{
                  category
                    .split(" ")
        ...
        <div class="flex items-center justify-center space-x-4">
          <div class="flex items-center space-x-2">
            ...
            >
              <svg
                class="fill-black opacity-30 transition-all hover:opacity-100"
                :class="!showSearchBar ? '' : 'opacity-100'"
            ...
            <input
              ...
              :class="
                showSearchBar
                  ? 'border-b-2 border-gray-600 px-1 w-40 md:w-56'
                  : 'w-0'
              "
            />
          </div>
          ...
          <select
            ...
            v-model="orderBy"
            class="text-xs cursor-pointer p-2 rounded-lg shadow-md transition-all hover:shadow-lg hover:bg-gray-100 outline-none"
          >
          ...
          <div v-if="!loadingProducts" class="flex space-x-2">
            <svg
              @click="orderAsc = orderBy != '' ? true : orderAsc"
              xmlns="http://www.w3.org/2000/svg"
              width="20"
              height="20"
              fill="#000"
              class="bi bi-chevron-double-down cursor-pointer transition-all hover:scale-125"
              viewBox="0 0 16 16"
              :class="!orderAsc || orderBy == '' ? 'opacity-30' : ''"
            >
              <path
                fill-rule="evenodd"
            ...
            <svg
              @click="orderAsc = orderBy != '' ? false : orderAsc"
              xmlns="http://www.w3.org/2000/svg"
              width="20"
              height="20"
              fill="#000"
    ...
    <div
      v-else
      class="container mx-auto flex items-baseline flex-wrap md:mt-4 lg:mt-8"
    >
      <div
      ...
      >
        <router-link v-bind:to="product.get_absolute_url" class="space-y-1">
          <img
            class="hover:grow hover:opacity-70 max-h-56 mx-auto"
            :src="product.get_thumbnail"
          />
          <div class="pt-3 flex items-center justify-between">
            <p class="font-bold mx-auto">{{ product.name }}</p>
          </div>
          <p class="text-gray-500">DH {{ product.price }}</p>
        </router-link>
      </div>
    </div>
  </section>
    ...
    const sortProducts = () => {
      products.sort((p1, p2) => {
        switch (orderBy.value) {
          case "title":
            if (orderAsc.value == true) return p1.title > p2.title ? 1 : -1;
            else return p1.title > p2.title ? -1 : 1;
            break;
          case "price":
            if (orderAsc.value == true) return p1.price > p2.price ? 1 : -1;
            else return p1.price > p2.price ? -1 : 1;
            break;
          case "rate":
            if (orderAsc.value == true)
              return p1.rating.rate > p2.rating.rate ? 1 : -1;
            else return p1.rating.rate > p2.rating.rate ? -1 : 1;
            break;
        }
      });
    };
    ...
    watch(showSearchBar, (value) => {
      if (value == true) {
        setTimeout(() => {
          const handleClick = (e) => {
            if (
              !["search-product-bar", "search-product-btn"].includes(
                e.target.id
              ) &&
              searchProduct.value.length === 0
            ) {
              searchProduct.value = "";
